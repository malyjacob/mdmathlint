...
22 条规则分为六个类别。运行 `mdmathlint --explain <规则代号>` 可查看任意规则的完整说明、错误示例和正确示例。
...
| **MDM019** | warning | 否 | `\ref{...}` / `\eqref{...}` 在本次检查输入集合中找不到匹配的 `\label{...}` |
| **MDM021** | warning | 否 | 超过 400 字符、花括号嵌套超过 12 层，或已配置宏使用超过 20 次的公式 |
...
| **MDM020** | warning | 否 | `\require`、`\bbox`、`\cssId`、`\class` 等 MathJax 专属扩展命令 |

### LLM 优化风格 — "AI 生成，mdmathlint 把关"

| 规则 | 默认级别 | 可修复 | 检测内容 |
|---|---|---|---|
| **MDM022** | **off** | 否 | 使用了 `$...$` / `$$...$$` 而 `\(...\)` / `\[...\]` 更稳健 |
| **MDM023** | warning | 否 | 同一文档同时混用 `$...$` 和 `\(...\)` 两种定界符 |
| **MDM024** | warning | 否 | 未知 LaTeX 命令——可能是 LLM 幻觉 |

启用 `MDM014` 后，轻量 Pandoc、Goldmark 和 Obsidian 定界符 adapter 会加入既有的 remark 与 markdown-it 对比。它们用于模拟可移植性相关的识别规则，并非嵌入完整目标渲染引擎。

在同一条命令中检查多个 Markdown 文件时，`MDM019` 会在整个输入集合中解析标签，因此一个章节中的 `\ref{...}` 可以引用另一章节定义的 `\label{...}`。
...
选项：
  ...
  --fix-prompt                  输出可喂回 LLM 的自然语言修复提示
  --preset <名称>               快捷预设：chatgpt | claude | deepseek
  --fast                        跳过 KaTeX 解析（仅结构性检查，速度更快）
  --watch                       输入文件变化时重新检查
...
**llm**——面向 LLM 的结构化 JSON，包含结构化 issues、bad/good 示例以及可喂回 AI Agent 的 `fix_prompt`：

```bash
mdmathlint answer.md --profile llm-output --format llm
```

输出包含 `pass`/`summary`、每条 issue 含 `snippet`/`examples[]`/`why`，以及 `fix_prompt` 字符串。
...
mdmathlint 从设计之初就考虑了 AI 生成内容的校验。除 `llm-output` profile 外，以下功能构成完整的 LLM 反馈闭环：

### `--format llm` 与 `--fix-prompt`
...
通过 Model Context Protocol 实现 Agent 原生工具调用：
...
| 工具 | 说明 |
|---|---|
| `lint_markdown` | 检查 + 可选自动修复一步完成。支持 `profile`、`rules`、`macros`、`fast`、`fix`。返回结构化 issues（含 bad/good 示例）+ `fix_prompt` + 可选 `fixed` 文本。 |
| `fix_markdown` | 应用安全自动修复（空格、空行）。返回 `fixed`、`changed`、`original`。 |
| `explain_rule` | 获取规则的完整说明 + bad/good 示例 |
| `list_rules` | 列出所有规则及其 profile 感知级别 |
...
跳过 KaTeX 解析——仅运行结构性检查（定界符、空格、格式）。适合 LLM 大批量输出的快速初筛：
...
仓库内置了可直接使用的 [`skill/`](skill/) 目录，含主 playbook（[SKILL.md](skill/SKILL.md)）和详细参考页面（[references/](skill/references/)）——放入任意 AI Agent 上下文即可即时了解全部功能。
