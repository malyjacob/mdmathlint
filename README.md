...
22 rules organized into six categories. Run `mdmathlint --explain <rule-id>` for full details on any rule, including bad and good examples.
...
### LLM-optimized style — "generated by AI, validated by mdmathlint"

| Rule | Default | Fixable | Detects |
|---|---|---|---|
| **MDM022** | **off** | no | `$...$` / `$$...$$` where `\(...\)` / `\[...\]` would be more portable |
| **MDM023** | warning | no | Same document mixing `$...$` and `\(...\)` delimiter styles |
...
| **`llm-output`** | AI-generated content quality gate | MDM015 → error, MDM005 → warning, MDM013 → error, MDM022 → info |
...
Options:
  ...
  --fix-prompt                  output a natural-language fix prompt for LLM consumption
  --preset <name>               shortcut: chatgpt | claude | deepseek
  --fast                        skip KaTeX validation (faster, structural checks only)
...
**llm** — LLM-optimized JSON with structured issues, bad/good examples, and a `fix_prompt` ready to feed back to an AI agent:

```bash
mdmathlint answer.md --profile llm-output --format llm
```

The output includes `pass`/`summary`, per-issue `snippet`/`examples[]`/`why`, and a `fix_prompt` string.
...
mdmathlint was designed with AI-generated content in mind. Beyond the `llm-output` profile, three features form a complete LLM feedback loop:

### `--format llm` and `--fix-prompt`

Two output modes for AI agent consumption:

```bash
# Structured JSON with fix_prompt embedded
echo "$markdown" | mdmathlint --stdin --format llm --profile llm-output

# Plain-text fix instructions ready to feed back to the LLM
...
Agent-native tool calling via the Model Context Protocol:
...
| Tool | Description |
|---|---|
| `lint_markdown` | Lint + optional auto-fix in one call. Supports `profile`, `rules`, `macros`, `fast`, `fix`. Returns structured issues with examples + `fix_prompt` + optional `fixed` text. |
| `fix_markdown` | Apply safe auto-fixes (spacing, blank lines). Returns `fixed`, `changed`, `original`. |
| `explain_rule` | Get full details + bad/good examples for a rule |
| `list_rules` | List all rules with profile-aware severities |

### `--preset` shortcuts

Three presets for common LLM platforms:
...
```bash
mdmathlint answer.md --preset deepseek
```

### `--fast` mode

Skip KaTeX parsing — only run structural checks (delimiters, spacing, formatting). Useful for quick first-pass screening of large LLM output batches:

```bash
mdmathlint "output/**/*.md" --profile llm-output --fast
```

### Agent skill directory

A ready-to-use [`skill/`](skill/) directory with a main playbook ([SKILL.md](skill/SKILL.md)) and detailed reference pages ([references/](skill/references/)) is included in the repository — drop it into any AI agent context for instant familiarity.
